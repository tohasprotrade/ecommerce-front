        ...
        <div className='text-sm flex items-center gap-2'>
          <span className='max-w-52 text-ellipsis line-clamp-1'>
            ...
            <span className='text-medium text-red-600'>
              {user.role === "SUPER_ADMIN" ? "(Owner)" : user.role === "ADMIN" ? "(Admin)" : ""}
            </span>
          </span>
        ...
        <div className='text-sm grid gap-1'>
...

            {
              user.role === 'SUPER_ADMIN' && (
                <Link onClick={handleClose} to={"/dashboard/report"} className='px-2 hover:bg-orange-200 py-1'>Report</Link>
              )
            }

            {
              user.role === 'SUPER_ADMIN' && (
                <Link onClick={handleClose} to={"/dashboard/users"} className='px-2 hover:bg-orange-200 py-1'>Manage Users</Link>
              )
            }
